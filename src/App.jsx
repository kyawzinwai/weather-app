...
function App() {
  ...
  useEffect(() => {
    ...
    if (data != null) {
      ...
      windMessage = data?.current?.wind_speed > 15
  ...
  return (
    <div className="App">
      <div className="screenContainer">
        <button disabled={loading} className='btnEnterZipCode' onClick={openModal}>Enter ZIP Code</button>
        {loading && <p>Loading...</p>}
        ...
        <div className="faqContainer">
          {faqData.map((item, index) => (
            <div key={index} className="faqItem" id={`faqItem-${index}`} onClick={() => toggleAnswer(index)}>
              ...
              <div className={`faqAnswer ${expandedItems.includes(index) ? 'active' : ''}`}>
                {answers[index]}
              </div>
            </div>
          ))}
